...
@Component({
  selector: 'app-wound-care',
  templateUrl: './wound-care.html',
  styleUrl: './wound-care.scss',
  imports: [CommonModule, MatCardModule, MatButtonModule, MatIconModule, MatExpansionModule, MatChipsModule]
})
export class WoundCare implements OnInit {
  topics: Topic[] = [];
  ...
  ngOnInit(): void {
    this.dataService.getTopics().subscribe(topics => {
      this.topics = topics.filter(t => t.category === 'wound-care');
    });
  }
